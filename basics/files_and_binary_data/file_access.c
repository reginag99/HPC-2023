    ...
    )
{
  ...
  FILE *file =  fopen("test_file.dat", "w"); // file är platsen där adressen till filen lagras
  ...
  fwrite((void*) a, sizeof(int), 8, file); //fwrite is the analog of assigning  bunch of bites to peice of the file
  fclose(file);
